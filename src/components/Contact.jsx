...
const Contact = () => {
    ...
    const sendEmail = (e) => {
        e.preventDefault();
        emailjs.sendForm('service_9ri5i65', 'template_zw1ley8', form.current, 'wTc3xyzTVf89qheHV')
        .then((result) => {
        ...
        <>
            <div id='Contact' className="container max-w-6xl py-5 my-7 lg:p-5">
                <motion.h1 
                    ...
                    viewport={{ once: true }} 
                    className='md:text-[48px] pb-10 text-[32px] text-black   dark:text-slate-300 font-Ramabhadra text-center'>Contact me
                </motion.h1>
                        ...
                        {Info.map((item) => (           
                            <div key={item.key} className="flex items-center flex-row lg:m-4">
                                <motion.div variants={zoomIn()} className="flex shrink">
                                    <svg xmlns="http://www.w3.org/2000/svg" className='flex w-[4rem] h-[4rem] fill-black stroke-black dark:fill-slate-200 dark:stroke-slate-200' width="24" height="24" viewBox={item.viewbox}>
                                        <path d={item.path} />
                                    </svg>     
                                </motion.div>
                                <motion.div variants={fadeIn('up' , 80)} className="flex flex-col p-2 m-2  self-center">
                                    <h1 className='text-[32px] text-black  dark:text-slate-300 font-serif'>{item.text1}</h1>                       
                                    <span className="text-md text-black dark:text-slate-300 ">{item.text2}</span>
                                </motion.div>
                            </div>
                        ))} 
                    </motion.div>
                        ...
                        <motion.form ref={form} onSubmit={sendEmail} variants={staggerContainer(0.1 , 0)} initial="hidden" whileInView="show" viewport={{ once: true }}>
                            <div className="flex flex-row-2 gap-5">
                                <div className="flex flex-col w-full">
                                    <motion.label for="name" variants={fadeIn('up' , 50)} transition = {{type : 'tween' , duration :    .5 }} className='text-black  dark:text-slate-300 text-[20px] py-1'>Full name</motion.label>           
                                ...
                                <div className="flex flex-col w-full">
                                    <motion.label for="email" variants={fadeIn('up' , 50)} transition = {{type : 'tween' , duration : .5 }} className='text-black    dark:text-slate-300 text-[20px] py-1'>Email</motion.label>           
                            ...
                            <div className="flex flex-col">
                                <motion.label for="message" variants={fadeIn('up' , 50)} transition = {{type : 'tween' , duration : .5 }} className='text-black  dark:text-slate-300 text-[20px] py-1'>Your Message</motion.label>     
