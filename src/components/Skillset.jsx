...
const Skillset = () => {  
        ...
        document.querySelectorAll(".card").forEach((b) => {
            ...
            b.addEventListener("mousemove", (e) => {
                ...
                if(!dark) {
                    ...
                    e.target.style.borderImage = `radial-gradient(30% 80% at ${x}px ${y}px , rgba(255,255,255,1), rgb(107 114 128) ) 1 / 1px / 0 stretch`;
                } else  {
    ...
    return (
        <>
            <div id="Skillset" className="container max-w-6xl py-[20px] my-5 lg:p-5 lg:py-20 md:py-20">
                <motion.h1  
                ...
                <motion.div 
                    ...
                    {Skillsets.map((item) => (
                        <motion.div     
                            ...
                            className=' card bg-zinc-200  md:p-3 p-2 border-gray-400 dark:border-gray-500 border-[1px] flex md:justify-start items-center justify-center  dark:bg-zinc-900' 
                        >
                                <svg xmlns="http://www.w3.org/2000/svg" className='w-11 h-11 pointer-events-none z-5 fill-black dark:fill-slate-200 bg-transparent' width="24" height="24" viewBox={item.viewbox}>
                                    <path d={item.path} />
                                </svg>
                                <p className='hidden md:block z-5 flex-initial pl-5 lg:text-[21px] pointer-events-none md:text-[16px] bg-transparent font-Ramabhadra text-black dark:text-slate-200'>{item.name}</p>
                        </motion.div>
                    ))}
                </motion.div>
            </div>
